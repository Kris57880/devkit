...
# from compressai.entropy_models import GaussianConditional # if the compressai!=1.1.5
...
class TestModel(CompressionModel):
    ...
    def forward(self, x, noisequant=False):
        ...
        anchor = torch.zeros_like(y).to(x)
        non_anchor = torch.zeros_like(y).to(x)
        ...
        ctx_params_anchor_split = torch.split(torch.zeros(B, C * 2, H, W).to(x),
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            scales_hat_split = torch.zeros_like(y_anchor).to(x)
            means_hat_split = torch.zeros_like(y_anchor).to(x)
    ...
    def compress(self, x):
        ...
        ctx_params_anchor_split = torch.split(torch.zeros(B, C * 2, H, W).to(x), [2 * i for i in self.groups[1:]], 1)
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            y_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor//2).to(x)
            means_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor//2).to(x)
            scales_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(x)
            y_anchor_decode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor).to(x)
            ...
            y_non_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(x)
            means_non_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(x)
            scales_non_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(x)
    ...
    def decompress(self, strings, shape):
        ...
        ctx_params_anchor = torch.zeros((B, self.M*2, z_hat.shape[2] * 4, z_hat.shape[3] * 4)).to(z_hat)
        ...
        for slice_index in range(len(self.groups) - 1):
            ...
            means_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(z_hat)
            scales_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(z_hat)
            y_anchor_decode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor).to(z_hat)
            ...
            means_non_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(z_hat)
            scales_non_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2).to(z_hat)
